...
import com.gyz.megaeyefamily.httprequest.GalDownLoadTask.GalDownLoadTaskListener;
...
public class GalHttpRequest {
	 ...
	 * @throw
	 */
	public static GalHttpRequest requestWithURL(Context context,
	 ...
	 * @throw
	 */
	public static void startDownLoadFile(Context context,
			GalDownloadParams params, GalDownLoadTaskListener listener) {
		if (listener == null) {
	...
	private HttpResponse requestHttp(boolean haveLastModified, boolean haveEtag) {
		if (galurl == null || GalStringUtil.isEmpty(galurl.getUrl())
				|| "null".equals(galurl.getUrl())) {
			LogUtil.i("galurl 为空");
			return null;
		}
		...
		try {
			if (writeTocache) {
				...
				if (!GalStringUtil.isEmpty(filepath)) {
					...
					if (GalDBHelper.getInstance(mContext).existURL(
							galurl.getUrl())) {
				...
				return new BufferedInputStream(response.getEntity()
						.getContent());
			}
		} catch (Exception e) {
			LogUtil.e(new Throwable().getStackTrace()[0].toString()
